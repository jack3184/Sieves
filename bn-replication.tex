...
Since we assume that $v_i \sim U[0, 1]$, this essentially means that we restrict $w_i^{1-\alpha} > y_i$.
...
\begin{align*}
	...
	\intertext{Rearranging yields}
	h_i^* &= \frac{w_i^{1/\alpha} - y_i v_i}{w_i^{1/\alpha} + w_i}.
	\intertext{Hence the labor supply function, ruling out corner solutions, is}
	\pi (y_i, w_i, v_i) &= \max \left\lbrace 0, \enskip \frac{w_i^{1/\alpha} - y_i v_i}{w_i^{1/\alpha} + w_i} \right\rbrace.
\end{align*}
We get $\bar{\pi}_i (y_i, w_i)$ by integrating $v_i$ out:
\begin{align*}
	\bar{\pi}_i (y_i, w_i) &= \int_0^1 \frac{w_i^{1/\alpha} - y_i v_i}{w_i^{1/\alpha} + w_i} \, d v_i = \left[ \frac{w_i^{1/\alpha} v_i}{w_i^{1/\alpha} + w_i} - \frac{y_i v_i^2}{2 \left( w_i^{1/\alpha} + w_i \right) } \right]_{v_i=0}^1 = \frac{1}{2} - \frac{y_i}{2 w_i}.
	\intertext{Furthermore, we calculate the inverse of $\pi$ w.r.t $v_i$ as follows:}
	\ell_i &= 1 - v_i - \frac{v_i y_i}{w_i} \\
	w_i \ell_i &= w_i - v_i w_i - v_i y_i = w_i - v_i (w_i + y_i) \\
	v_i (w_i + y_i) &= w_i - w_i \ell_i = w_i (1-\ell_i) \\
